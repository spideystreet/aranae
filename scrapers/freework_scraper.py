...
from scrapers.utils import extract_json_ld, get_headers, init_job_details, polite_sleep
...
def fetch_jobs(page: int = 1, url: str = FREEWORK_BASE_URL) -> list[dict]:
    ...
    if "fw-text-highlight" not in response.text:
        print(
            f"Warning: 'fw-text-highlight' not found in response text. Content length: {len(response.text)}"
        )
        # Debug: save to file for inspection
        with Path(f"debug_page_{page}.html").open("w") as f:
    ...
    for i, link_tag in enumerate(links):
        try:
            ...
            publication_date_fallback = details.get("publication_date") or date_posted
            ...
            VALID_CONTRACTS = {"freelance", "cdi", "cdd", "alternance", "stage"}
            ...
            job_payload = build_freework_job_payload(
                ...
                contracts=contracts,
                skills=skills,
                publication_date_fallback=publication_date_fallback,
            )
...
if __name__ == "__main__":
    ...
    if data:
        ...
        for i, job in enumerate(data[:5]):
            ...
            print(
                f"  Income: {job['income']} | Duration: {job['duration']} | XP: {job['experience_level']}"
            )
            print("---")
